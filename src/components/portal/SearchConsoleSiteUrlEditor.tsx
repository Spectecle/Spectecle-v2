...
export function SearchConsoleSiteUrlEditor({
  organizationId,
  domain,
  orgName,
  websiteUrl,
  ...
  const [value, setValue] = useState(currentSiteUrl ?? "");
    ...
    try {
      const res = await fetch("/api/portal/admin/organizations", {
        ...
        body: JSON.stringify({
          ...
          name: orgName,
          websiteUrl,
          searchConsoleSiteUrl: value.trim() || null,
        }),
      });
      if (!res.ok) {
      ...
      <div className="mb-4">
        <div className="flex items-center gap-1.5 flex-wrap">
          <input
            ...
            autoFocus
            className="bg-[var(--portal-input-bg)] border border-[var(--portal-border-strong)] text-[var(--portal-text-primary)] px-2 py-1 text-sm outline-none focus:border-[#cb7c46]/50 w-64"
          />
          <button
            type="button"
            onClick={handleSave}
  ...
  return (
    ...
    >
      <span className="text-sm text-[var(--portal-text-muted)]">
        Search Console:{" "}
        <span className="text-[var(--portal-text-secondary)]">{currentSiteUrl ?? "Not connected"}</span>
      </span>
