...
export function GoogleReviewUrlEditor({
  ...
  const [value, setValue] = useState(currentReviewUrl ?? "");
    ...
    try {
      const res = await fetch("/api/portal/admin/organizations", {
        ...
        body: JSON.stringify({
          ...
          name: orgName,
          websiteUrl,
          googleReviewUrl: value.trim() || null,
        }),
      });
      if (!res.ok) {
      ...
      <div className="mb-4">
        <div className="flex items-center gap-1.5 flex-wrap">
          <input
            ...
            placeholder="Google review link, e.g. https://g.page/r/.../review"
            autoFocus
            className="bg-[var(--portal-input-bg)] border border-[var(--portal-border-strong)] text-[var(--portal-text-primary)] px-2 py-1 text-sm outline-none focus:border-[#cb7c46]/50 w-80"
          />
          <button
            type="button"
            onClick={handleSave}
  ...
  return (
    ...
    >
      <span className="text-sm text-[var(--portal-text-muted)]">
        Google Review Link:{" "}
        <span className="text-[var(--portal-text-secondary)]">{currentReviewUrl ?? "Not set"}</span>
      </span>
