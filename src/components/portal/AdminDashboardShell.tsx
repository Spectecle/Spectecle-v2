...
const NAV_ITEMS = [
  {
    ...
    href: "/portal/admin",
    isActive: (pathname: string, section: string | null) =>
      pathname === "/portal/admin" && (section === null || section === "dashboard"),
  },
  {
    value: "requests",
    label: "Requests",
    icon: Inbox,
    href: "/portal/admin?section=requests",
    isActive: (pathname: string, section: string | null) =>
      pathname === "/portal/admin" && section === "requests",
  },
  {
    value: "clients",
    label: "Clients",
    ...
    isActive: (pathname: string, section: string | null) =>
      (pathname === "/portal/admin" && section === "users") ||
      ...
      >
        <div className="flex items-center justify-between p-3 border-b border-[var(--portal-border)]">
          ...
          <button
            ...
            className="hidden lg:block p-1.5 text-[var(--portal-text-muted)] hover:text-[var(--portal-text-primary)] cursor-pointer transition-colors ml-auto"
        ...
        <nav className="flex-1 p-3 space-y-1">
          {NAV_ITEMS.map((item) => {
            ...
            const isActive = item.isActive(pathname, section);
            return (
              <Link
                key={item.value}
                href={item.href}
