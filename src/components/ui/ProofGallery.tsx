      ...
      <div className="grid sm:grid-cols-3 gap-x-10 gap-y-10">
        {featured.map((project) => (
          <Link key={project.slug} href={`/work/${project.slug}`} className="group block">
            ...
            <h3 className="mt-1 text-lg font-light text-[var(--site-text-primary)] group-hover:text-[#cb7c46] transition-colors" style={{ fontFamily: "var(--font-serif)" }}>
              {project.title}
            </h3>
            <p className="mt-1 text-sm font-semibold text-[#cb7c46]">{project.cardResult}</p>
