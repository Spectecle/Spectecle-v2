    ...
    <>
      ...
      <section className="pt-32 pb-10 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="mt-4 text-[#c69947] font-medium text-lg">{project.tagline}</p>
          <a
            href={project.liveUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-4 inline-flex items-center gap-2 text-[var(--site-text-secondary)] hover:text-[var(--site-text-primary)] transition-colors text-sm group"
          >
      ...
      <section className="py-16 px-6 border-b border-[var(--site-border)]">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8">
          {project.heroMetrics.map((m) => (
            <div key={m.label}>
              <div className="text-3xl md:text-4xl font-light text-[#c69947]" style={{ fontFamily: "var(--font-serif)" }}>
                {m.value}
              </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-10">
            {project.features.map((f, i) => {
              ...
              return (
                ...
                >
                  <Icon className="w-5 h-5 text-[#c69947] mb-3" />
            ...
            <div className="space-y-3">
              {project.deliverables.map((d) => (
                <div key={d} className="flex items-center gap-3 text-sm text-[var(--site-text-secondary)]">
                  <CheckCircle2 className="w-4 h-4 shrink-0 text-[#c69947]" />
                  {d}
                </div>
              ))}
            </div>
          ...
          <Reveal>
            ...
            <Link href={`/work/${nextProject.slug}`} className="group flex items-center justify-between">
              <div>
                ...
                <h3 className="text-3xl font-light text-[var(--site-text-primary)] mt-1 group-hover:text-[#c69947] transition-colors" style={{ fontFamily: "var(--font-serif)" }}>
                  {nextProject.title}
                </h3>
                <p className="text-[var(--site-text-secondary)] text-sm mt-1">{nextProject.domain}</p>
              </div>
              <ChevronRight className="w-6 h-6 text-[var(--site-text-muted)] group-hover:text-[#c69947] transition-colors shrink-0" />
            </Link>
          </Reveal>
        </div>
      </section>
        ...
        <Reveal>
          <div className="max-w-3xl mx-auto text-center">
            <h2 className="text-5xl md:text-6xl font-light text-[var(--site-text-primary)]" style={{ fontFamily: "var(--font-serif)" }}>
              Ready for results <span className="italic text-[#c69947]">like these?</span>
            </h2>
