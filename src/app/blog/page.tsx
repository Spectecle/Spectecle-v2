  ...
  if (!project) {
    return (
      <div className={`relative w-full ${size === "compact" ? "h-40" : "h-56 md:h-72"} overflow-hidden bg-[var(--site-surface)] border border-[var(--site-border)] flex items-center justify-center`}>
        <span className="text-2xl text-[#cb7c46]">✦</span>
      </div>
    );
  }
    ...
    <>
      ...
      <section className="pt-40 pb-12 px-6">
        <div className="max-w-3xl mx-auto text-center">
          ...
          >
            Guides that <span className="italic text-[#cb7c46]">actually work.</span>
          </motion.h1>
          ...
          <Reveal>
            <Link href={`/blog/${featured.slug}`} className="group grid lg:grid-cols-[1fr_1fr] gap-8 items-center">
              <div className="relative">
                <span
                  className="absolute top-4 left-4 z-10 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[#1e1e1e] bg-[#cb7c46]"
                >
                  Featured
                </span>
              ...
              <div>
                ...
                <h2
                  className="text-3xl md:text-4xl font-light text-[var(--site-text-primary)] leading-snug group-hover:text-[#cb7c46] transition-colors duration-300"
                  style={{ fontFamily: "var(--font-serif)" }}
                >
                  {featured.title}
                </h2>
            ...
            {rest.map((post, i) => (
              <Reveal key={post.slug} delay={i * 0.07}>
                <Link href={`/blog/${post.slug}`} className="group block">
                  ...
                  <h3
                    className="mt-3 text-xl font-light text-[var(--site-text-primary)] leading-snug group-hover:text-[#cb7c46] transition-colors duration-300"
                    style={{ fontFamily: "var(--font-serif)" }}
                  >
                    {post.title}
                  </h3>
        ...
        <Reveal>
          <div className="max-w-3xl mx-auto text-center">
            <h2 className="text-5xl md:text-6xl font-light text-[var(--site-text-primary)]" style={{ fontFamily: "var(--font-serif)" }}>
              Ready to grow your <span className="italic text-[#cb7c46]">business?</span>
            </h2>
